...
function __isMonitorSnapshotPayload__(obj) {
  if (!obj || typeof obj !== 'object' || Array.isArray(obj)) return false;
  var hasAssets = Array.isArray(obj.assets);
  var hasPositions = Array.isArray(obj.positions);
  var hasTotals = !!(obj.totals && typeof obj.totals === 'object' && !Array.isArray(obj.totals));
  return hasTotals && (hasAssets || hasPositions);
}

function __readMonitorSnapshotChunkedByManifest__(props, keyBase, manifestKey, partSuffix, mode) {
  try {
    var rawManifest = props && props[manifestKey];
    if (!rawManifest) return { ok: false, reason: 'MANIFEST_MISSING' };

    var man;
    try {
      man = JSON.parse(rawManifest);
    } catch (_) {
      return { ok: false, reason: 'MANIFEST_PARSE_FAILED' };
    }

    var partKeys = [];
    var partMode = 'unknown';

    if (mode === 'v2') {
      if (!man || typeof man !== 'object' || Array.isArray(man)) {
        return { ok: false, reason: 'V2_MANIFEST_TYPE_INVALID' };
      }
      if (man.schema !== 'monitor_snapshot_chunk_manifest') {
        return { ok: false, reason: 'V2_SCHEMA_INVALID' };
      }
      if (Number(man.version) !== 2) {
        return { ok: false, reason: 'V2_VERSION_INVALID' };
      }
      if (man.snapshotType !== 'FULL') {
        return { ok: false, reason: 'V2_SNAPSHOT_TYPE_INVALID' };
      }
      if (!Array.isArray(man.parts) || man.parts.length < 1) {
        return { ok: false, reason: 'V2_PARTS_ARRAY_INVALID' };
      }
      var v2PartCount = Number(man.partCount);
      if (!Number.isInteger(v2PartCount) || v2PartCount < 1) {
        return { ok: false, reason: 'V2_PARTCOUNT_INVALID' };
      }
      if (v2PartCount !== man.parts.length) {
        return { ok: false, reason: 'V2_PARTCOUNT_MISMATCH' };
      }
      partKeys = man.parts.map(function(k){ return String(k || '').trim(); }).filter(Boolean);
      if (partKeys.length !== man.parts.length) {
        return { ok: false, reason: 'V2_PARTKEY_INVALID' };
      }
      partMode = 'array';
    } else {
      // Legacy V1 path: accept numeric part count or explicit part-key array.
      if (Array.isArray(man.parts) && man.parts.length > 0) {
        partKeys = man.parts.map(function(k){ return String(k || '').trim(); }).filter(Boolean);
        if (partKeys.length !== man.parts.length) {
          return { ok: false, reason: 'V1_PARTKEY_INVALID' };
        }
        partMode = 'array';
      } else {
        ...
        if (Number.isFinite(n) && n > 0) {
          for (var i = 1; i <= n; i++) {
            partKeys.push(keyBase + '_P' + i + partSuffix);
          }
          partMode = 'numeric';
        }
      }
      if (!partKeys.length) {
        return { ok: false, reason: 'V1_PARTS_INVALID' };
      }
    }
    ...
    for (var j = 0; j < partKeys.length; j++) {
      ...
      if (typeof chunk !== 'string' || !chunk) {
        return { ok: false, reason: 'CHUNK_MISSING_PART', partKey: pk, partMode: partMode };
      }
      buf += chunk;
    }
    ...
    try {
      ...
      return { ok: false, reason: 'PAYLOAD_PARSE_FAILED', partMode: partMode };
    }

    if (!__isMonitorSnapshotPayload__(payload)) {
      return { ok: false, reason: 'PAYLOAD_SHAPE_INVALID', partMode: partMode };
    }

    return { ok: true, snapshot: payload, manifest: man, partMode: partMode, partCount: partKeys.length };
  } catch (e) {
    return { ok: false, reason: 'CHUNK_READ_FAILED', error: (e && e.message ? e.message : String(e || 'unknown')) };
  }
}

function __resolveMonitorSnapshotFromProps__(props, keyBase) {
  var out = { ok: false, source: 'none', snapshot: null, warnings: [], error: null };
  if (!props || typeof props !== 'object') {
    out.error = 'PROPS_INVALID';
    return out;
  }

  var v2ManifestKey = keyBase + '_MANIFEST_V2';
  var v1ManifestKey = keyBase + '_MANIFEST_V1';

  var v2 = __readMonitorSnapshotChunkedByManifest__(props, keyBase, v2ManifestKey, '_V2', 'v2');
  if (v2.ok) {
    out.ok = true;
    out.source = 'v2_chunked';
    out.snapshot = v2.snapshot;
    return out;
  }
  if (props[v2ManifestKey]) {
    out.warnings.push('V2_INVALID_' + String(v2.reason || 'UNKNOWN'));
  }

  var v1 = __readMonitorSnapshotChunkedByManifest__(props, keyBase, v1ManifestKey, '_V1', 'v1');
  if (v1.ok) {
    out.ok = true;
    out.source = (v1.partMode === 'array') ? 'legacy_v1_chunked_array' : 'legacy_v1_chunked_numeric';
    out.snapshot = v1.snapshot;
    out.warnings.push('LEGACY_V1_SOURCE_USED');
    return out;
  }
  if (props[v1ManifestKey]) {
    out.warnings.push('V1_INVALID_' + String(v1.reason || 'UNKNOWN'));
  }

  var raw = props[keyBase];
  if (typeof raw === 'string' && raw) {
    var parsed;
    try {
      parsed = JSON.parse(raw);
    } catch (_) {
      out.warnings.push('RAW_PARSE_FAILED');
      out.error = 'NO_VALID_SNAPSHOT_SOURCE';
      return out;
    }

    if (__isMonitorSnapshotPayload__(parsed)) {
      out.ok = true;
      out.source = 'raw';
      out.snapshot = parsed;
      out.warnings.push('LEGACY_RAW_SOURCE_USED');
      return out;
    }

    out.warnings.push('RAW_PAYLOAD_SHAPE_INVALID');
  }
...
function hostGetMonitorSnapshot() {
  try {
    ...
    var resolved = __resolveMonitorSnapshotFromProps__(props, 'MONITOR_SNAPSHOT_FULL');
    var snap = resolved.snapshot || {};

    if (!resolved.ok) {
      try { Logger.log('[MON_SNAPSHOT][READ][ERR] source=none reason=' + (resolved.error || 'unknown')); } catch (_) {}
    } else if (resolved.source !== 'v2_chunked') {
      try { Logger.log('[MON_SNAPSHOT][READ][WARN] source=' + resolved.source + ' fallback=' + (resolved.warnings || []).join('|')); } catch (_) {}
    } else {
      try { Logger.log('[MON_SNAPSHOT][READ][OK] source=' + resolved.source); } catch (_) {}
    }
...
function hostGetPerformanceKpis(query, filters) {
  ...
  try {
    ...
    return RisontisCoreLibrary.PerformanceService.getPerformanceKpisWithProps(dto, query || {}, filters || null);
  } catch (e) {
...
function hostGetPerformanceKpisMulti() {
  try {
    ...
    const fullKey = 'MERGED_DATASET_90D_' + (dto.years || []).map(function(y){ return y.year + '_' + y.partKeys.length; }).join('__');

    // Load or build merged dataset once (bounded 90d base for sidebar KPI ranges)
    var merged = null;
    ...
    if (!merged) {
      ...
      merged = RisontisCoreLibrary.TradesService.getTradesDataWithProps(dto, { days: 90 }, null);
...
function hostGetMonitorSnapshotRaw() {
  try {
    ...
    if (resolved.source !== 'v2_chunked') {
      try { Logger.log('[MON_SNAPSHOT][READ][WARN] raw_endpoint source=' + resolved.source + ' fallback=' + (resolved.warnings || []).join('|')); } catch (_) {}
    }
    ...
    try { Logger.log('[Host#hostGetMonitorSnapshotRaw] source=' + resolved.source + ' len=' + payload.length + ' first200=' + payload.substring(0,200)); } catch(_) {}
    return payload;
  } catch (e) {
...
function __dpReadFullChunked__(props, keyBase) {
  var resolved = __resolveMonitorSnapshotFromProps__(props, keyBase || 'MONITOR_SNAPSHOT_FULL');
  return resolved && resolved.snapshot ? resolved.snapshot : null;
}
...
function hostGetAvailableRange() {
  try {
    ...
    var navEarliest = null;
    var navLatest = null;
    var tradeEarliest = null;
    var tradeLatest = null;
    var tradeErrorCode = null;
    var tradeError = null;
    var source = 'none';
    ...
    try {
      ...
      if (!rows.length && navDto && navDto.summary && navDto.summary.first && navDto.summary.last) {
        navEarliest = navDto.summary.first;
        navLatest = navDto.summary.last;
      } else if (rows.length) {
        navEarliest = rows[0].dateISO || rows[0].date || null;
        navLatest = rows[rows.length - 1].dateISO || rows[rows.length - 1].date || null;
      }
    } catch (_) {
      navEarliest = navEarliest || null;
      navLatest = navLatest || null;
    }
    // --- Trades range (canonical for trade analytics availability) ---
    try {
      if (globalThis.RisontisCoreLibrary &&
          RisontisCoreLibrary.TradesService &&
          typeof RisontisCoreLibrary.TradesService.getTradeBoundsWithProps === 'function') {

        var bounds = RisontisCoreLibrary.TradesService.getTradeBoundsWithProps(tradesDto);
        if (bounds && bounds.ok === true) {
          tradeEarliest = bounds.earliest || null;
          tradeLatest = bounds.latest || null;
          source = (tradeEarliest && tradeLatest) ? String(bounds.source || 'trade-bounds') : 'none';
        } else {
          tradeErrorCode = (bounds && bounds.errorCode) ? String(bounds.errorCode) : 'TRADES_BOUNDS_INVALID';
          tradeError = (bounds && bounds.error) ? String(bounds.error) : 'getTradeBoundsWithProps returned ok=false';
        }
      } else {
        tradeErrorCode = 'TRADES_BOUNDS_UNAVAILABLE';
        tradeError = 'TradesService.getTradeBoundsWithProps unavailable';
      }
    } catch (eTrades) {
      tradeErrorCode = 'TRADES_RANGE_EXCEPTION';
      tradeError = (eTrades && eTrades.message) ? eTrades.message : String(eTrades || 'unknown');
    }

    var hasArchive = !!(tradeEarliest && tradeLatest);

    return {
      ok: true,
      earliest: tradeEarliest,
      latest: tradeLatest,
      hasArchive: hasArchive,
      source: source,
      tradeRangeValid: hasArchive,
      tradeErrorCode: tradeErrorCode,
      tradeError: tradeError,
      navEarliest: navEarliest,
      navLatest: navLatest
    };
