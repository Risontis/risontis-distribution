...
function hostSyncLicense_() {
  try {
    ...
    if (license && verifyLicensePayload_(license)) {
      var dp = PropertiesService.getDocumentProperties();
...
function resolveLicenseStatusViaLibrary_() {
  ...
  var signatureOk = verifyLicensePayload_(parsed);
  var binding = buildLicenseBinding_();
  ...
  try {
    ...
    if (raw) {
      try {
        ...
        if (verifyLicensePayload_(cached)) {
          return { ok: true, license: cached };
        }
      } catch (_) {}
    }
    var refreshed = hostSyncLicense_();
    if (refreshed && refreshed.ok) {
      return refreshed;
    }
    return {
      ok: false,
      issues: (refreshed && refreshed.issues) || [ (refreshed && refreshed.error) || 'License not verified' ]
    };
  } catch (e) {
