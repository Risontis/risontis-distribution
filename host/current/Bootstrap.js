...
/**
 * HSU PRECHECK (read-only): compare canonical host.json with runtime versions.
 */
function hostCheckForUpdates_() {
  var status = {
    ok: false,
    hostUpToDate: false,
    libraryUpToDate: false,
    currentHostVersion: HOST_VERSION,
    targetHostVersion: null
  };
  const HOST_METADATA_URL = 'https://raw.githubusercontent.com/risontis/risontis-distribution/main/host/current/host.json';
  ...
  try {
    ...
    status.targetHostVersion = hostJson && hostJson.hostVersion ? String(hostJson.hostVersion) : null;
    status.hostUpToDate = status.targetHostVersion === status.currentHostVersion;
    status.libraryUpToDate = true;
    status.ok = status.hostUpToDate;
    Logger.log('[HSU][Precheck] hostUpToDate=' + status.hostUpToDate +
      ' libraryUpToDate=' + status.libraryUpToDate +
      ' currentHostVersion=' + status.currentHostVersion +
      ' targetHostVersion=' + (status.targetHostVersion || 'n/a'));
  } catch (e) {
    status.ok = false;
    status.hostUpToDate = false;
    status.libraryUpToDate = false;
    Logger.log('[HSU][Precheck][ERR] ' + (e && e.message ? e.message : e));
  }
...
/**
 * Public menu entrypoint for HSU precheck.
 */
function hostCheckForUpdatesFromMenu_() {
  return hostCheckForUpdates_();
}

function hostCheckForUpdatesFromMenu() {
  var status;
  try {
    status = hostCheckForUpdates_();
  } catch (_) {
    status = null;
  }
  try {
    if (!status || typeof status !== 'object' || typeof status.hostUpToDate !== 'boolean') {
    ...
    if (status.hostUpToDate) {
      var ss = SpreadsheetApp.getActiveSpreadsheet();
      if (ss) {
        ss.toast('Your installation is up to date.', 'Risontis', 6);
      }
    } else {
      var body = [
        'An update is available for your Risontis installation.',
        'Current version: ' + (status.currentHostVersion || 'n/a'),
        'Available version: ' + (status.targetHostVersion || 'n/a'),
        'No changes have been made without your confirmation.'
      ].join('\n');
      var firstConfirm = SpreadsheetApp.getUi().alert(
        'Risontis Update Available',
        body,
        SpreadsheetApp.getUi().ButtonSet.OK_CANCEL
      );
      if (firstConfirm !== SpreadsheetApp.getUi().Button.OK) {
        return status;
      }
      var secondConfirm = SpreadsheetApp.getUi().alert(
        'Confirm Update',
        'This will apply the update now. The spreadsheet may need to be reloaded.\nDo you want to continue?',
        SpreadsheetApp.getUi().ButtonSet.OK_CANCEL
      );
      if (secondConfirm !== SpreadsheetApp.getUi().Button.OK) {
        return status;
      }
      hostApplyUpdateFromMenu();
    }
  } catch (_) {
    try {
      SpreadsheetApp.getUi().alert(
...
function hostApplyUpdateFromMenu() {
  Logger.log('[HSU][Apply] Apply Update requested');
  var status = { ok: false, requested: true, prepared: false, applied: false };
  try {
    hostPrepareApplyUpdate_();
    status.prepared = true;
    status.payload = hostResolveUpdatePayload_();
    hostExecuteUpdatePayload_(status.payload);
    status.applied = true;
    status.ok = true;
    Logger.log('[HSU][Apply] Update applied (target=' + (status.payload && status.payload.targetHostVersion ? status.payload.targetHostVersion : 'n/a') + ')');
  } catch (e) {
    status.ok = false;
    status.error = (e && e.message) ? e.message : String(e || 'unknown');
    Logger.log('[HSU][Apply][ERR] ' + status.error);
    try {
      SpreadsheetApp.getUi().alert(
        'Risontis Update Failed',
...
function inferUpdateFileType_(name) {
  var n = String(name || '').toLowerCase();
  if (n === 'appsscript.json' || /appsscript\.json$/i.test(n)) return 'JSON';
  if (/\.html$/i.test(n)) return 'HTML';
  return 'SERVER_JS';
}

function hostExecuteUpdatePayload_(payload) {
  if (!payload || !Array.isArray(payload.files) || !payload.files.length) {
    throw new Error('Update payload missing files');
  }
  var scriptId = ScriptApp.getScriptId && ScriptApp.getScriptId();
  if (!scriptId) throw new Error('Apply Update failed: Script ID unavailable in this execution context.');
  ...
  var files = payload.files.map(function(file) {
    ...
    if (!name) throw new Error('Update file name missing');
    var source = file && typeof file.content === 'string' ? file.content : '';
    if (!source) throw new Error('Update file content missing: ' + name);
    return {
      name: name,
      type: inferUpdateFileType_(name),
      source: source
    };
  });
  ...
  var code = res.getResponseCode();
  if (code !== 200) {
    throw new Error('Apply update failed with status ' + code);
  }
  return true;
}

function hostPrepareApplyUpdate_() {
  var scriptId = ScriptApp.getScriptId && ScriptApp.getScriptId();
  if (!scriptId) throw new Error('Apply Update failed: Script ID unavailable in this execution context.');
  var token = ScriptApp.getOAuthToken();
  var url = 'https://script.googleapis.com/v1/projects/' + encodeURIComponent(scriptId) + '/content';
  ...
  var code = res.getResponseCode();
  if (code !== 200) {
    throw new Error('Apply update auth check failed with status ' + code);
  }
  return true;
}

function hostResolveUpdatePayload_() {
  var url = 'https://raw.githubusercontent.com/risontis/risontis-distribution/main/host/current/host.json';
  var res = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
  var code = res.getResponseCode();
  if (code !== 200) throw new Error('host.json fetch failed with status ' + code);
  var hostJson = JSON.parse(res.getContentText());
  var target = hostJson && hostJson.hostVersion ? String(hostJson.hostVersion) : null;
  if (!target) throw new Error('host.json missing hostVersion');
  if (target === HOST_VERSION) throw new Error('Host version is already up to date');

  var files = (hostJson && hostJson.files) ? hostJson.files : [];
  if (!Array.isArray(files) || !files.length) throw new Error('host.json files list missing');
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    if (!fileUrl) throw new Error('Update file URL missing at index ' + i);

    var fileRes = UrlFetchApp.fetch(fileUrl, { muteHttpExceptions: true });
    var fileCode = fileRes.getResponseCode();
    if (fileCode !== 200) throw new Error('Update file fetch failed (' + fileCode + '): ' + (name || fileUrl));
    var content = fileRes.getContentText();
    if (!content) throw new Error('Update file empty: ' + (name || fileUrl));

    var isManifest = (name === 'appsscript.json') || /appsscript\.json$/i.test(fileUrl);
    if (isManifest) {
      try { JSON.parse(content); } catch (e) {
        throw new Error('appsscript.json invalid JSON');
      }
    }
    fetched.push({ name: name, url: fileUrl, content: content });
  }

  return { targetHostVersion: target, files: fetched };
}
...
function RunTradingSystem() {
  ...
  try {
    ...
    if (!Number.isFinite(lastEq) && !Number.isFinite(prevEq)) {
      ...
      Logger.log('[Bootstrap] Initializing equity state for cold install from STARTING_CASH_EUR=' + startingCash);
